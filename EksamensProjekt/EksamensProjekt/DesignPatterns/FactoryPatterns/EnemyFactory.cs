...
namespace EksamensProjekt.DesignPatterns.FactoryPatterns
{
    public class EnemyFactory
    {
        ...
        public Enemy CreateEnemy(Vector2 position, List<Vector2> path, float speed, float tileSize, bool isStrong)
        {
            if (isStrong)
            {
                return new Enemy(strongEnemyTexture, position, path, speed, tileSize, 10, 5, 1); // Strong enemy with more health
            }
            else
            {
                return new Enemy(normalEnemyTexture, position, path, speed, tileSize, 2, 1, 1); // Normal enemy
            }
        }
    }
}
